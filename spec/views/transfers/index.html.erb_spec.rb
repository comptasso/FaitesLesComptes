...
describe 'transfer/index' do
  ...
  before(:each) do
    ...
    @transfers=[]
    @transfers << stub_model(Transfer, name: 'Magasin', comment: 'la caisse du magasin')
    @transfers << stub_model(Transfer, name: 'Entrepot')
    @transfers.each do |b|
  ...
  context 'mise en page générale' do
    ...
    it "should have title h3" do
      page.find('h3').should have_content 'Liste des virements'
    end
  ...
  context 'test de l affichage de l icone destroy' do
    it "with a line, row should not propose supprimer" do
      @transfers.first.stub_chain(:cash_controls, :empty?).and_return(false)
      render
