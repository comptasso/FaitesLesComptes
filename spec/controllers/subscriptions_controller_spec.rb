...
describe SubscriptionsController do
  ...
  describe 'POST index', wip:true do
    ...
    it 'récupère les subscriptions qui ont des écritures à passer' do
      @o.should_receive(:subscriptions).and_return(@ar = double(Arel))
      @ar.should_receive(:all).and_return([sub])
      get :index, {}, valid_session
    end

    it 'assigne les subscriptions' do
      @o.stub_chain(:subscriptions, :all, :select).and_return([sub, sub])
      get :index, {}, valid_session
    ...
    it 'sans subscription en retard, rend un flash' do
      @o.stub_chain(:subscriptions, :all, :select).and_return([])
      get :index, {}, valid_session
