module Jobs
  ...
  # et un bloc.
  # 
  class VirtualBookPdfFiller < BasePdfFiller
    ...
    def set_document(options)
      @bank_account = @export_pdf.exportable
      @period = Period.find(options[:period_id])
      
      @virtual_book = @bank_account.virtual_book
      if options[:mois] == 'tous'
        @document = Extract::BankAccount.new(@virtual_book, @period)
      else
        @document = Extract::MonthlyBankAccount.new(@virtual_book, year:options[:an], month:options[:mois])
      end
