...
class Period < ActiveRecord::Base
  ...
  def previous_period
    Period.first(:conditions=>['structure_id = ? AND close_date < ?', self.structure_id, self.start_date],
  ...
  def next_period
    Period.first(:conditions=>['structure_id = ? AND start_date > ?', self.structure_id, self.close_date],
...
#    raise ArgumentError unless self.accounts.where('acc_number= ?', acc_number_to)
  ...
  def should_not_have_more_than_two_open_periods
    if self.structure.nb_open_periods >= 2
  ...
  # à la structure et des mois qui sont dans months
  ...
  def create_journals
    ...
    return if self.structure.journal_lists.empty?
    
    # pour chaque journal
    self.structure.journal_lists.each do |jl|
      month=0
