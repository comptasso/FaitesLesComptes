...
class DestinationsController < ApplicationController 
  ...
  #
  def index
    @sd = Stats::StatsDestinations.new(@period, [@filter])
    send_export_token

    respond_to do |format| 
      format.html
      format.js
      format.csv { send_data @sd.to_csv, filename:export_filename(nil, :csv, 'Statistiques par nature')  }  
      format.xls { send_data @sd.to_xls, filename:export_filename(nil, :csv, 'Statistiques par nature')  }
    end
  end
